...
elseif handles.bG2C.Value==1         %perform gray to color 
    if handles.bGCfreq.Value           %by frequency mapping
        ...
        if size(unique([LPC BPC HPC])) < 3
            errordlg('Please use different colors for different bands.','Convert Error','modal');
        else
            %%PROCEDURE
            ...
            if Lfreq>=Hfreq
            errordlg('Low cut off frequency should be less thatn the hight cut off frequency.','Convert Error','modal');
            histo=0;
            OutIma=0;
            else
