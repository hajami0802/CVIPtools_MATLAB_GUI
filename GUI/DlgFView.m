...
%%Revision 1.0 /Hridoy --  Changes the file format and locations of the
%%file
...
%uiwait(msgbox({'Save the features to the following directory:';'GUI/Features'},'Warning','warn'))

%open file selection dialog box to input image
[filename, pathname, index] = uiputfile({'*.csv','Comma delimited(*.csv)';...
    '*.xls','Excel 97-2003 Workbook (*.xls)'},...
    'Save Features as','GUI\Features'); %mulitple file selection option is OFF, single image file only 
% '*.xls','Excel 97-2003 Workbook (*.xls)'}, ...
...
%         outFile = [pwd '\GUI\' '\Features\' filename];
...
%         end
           ...
           name = getappdata(0,'name'); %get the name of the object
           name=which(name);
          %T = readtable(name);
%            filename=char(strcat(filename,'.csv'))
           %file1 = [pwd '\GUI\' '\Features\' name ];
           %file2 = [pwd '\GUI\' '\Features\'];
           %file2='G:\TOOLBOX RELEASE 3.61\GUI';
%            [~, ~, raw] = xlsread(name);
           %folder = '\GUI\Features\'
            %filename=char(strcat(filename,'.csv'));
            %filename=fullfile(folder,filename);
            %writetable(T,filename)
            movefile(name,filename)
            
            copyfile(filename,pathname,'f')
            name=filename;
            setappdata(0,'name',name);
%           name = [pwd '\GUI\' '\Features\' name];
%           [~, ~, raw]=xlsread(name);
%           csvwrite(filename,name);
...
%            tf = strcmp(name,file2);
%             if tf==1
%                 
%               name = [pwd '\GUI\' '\Features\' name];
%             else
%                name = [pwd '\GUI\' '\Features\' filename]; 
%             end
            [~, ~, raw] = xlsread(name);
            %filename=split(filename,'.csv');
%             filename= name(strlength(filename) > 0);
%             folder = 'G:\TOOLBOX RELEASE 3.61\GUI'
%             filename=char(strcat(filename,'.xls'));
...
%         outFile = [pwd '\GUI\'  '\Features\' filename];
%         if ~strcmp(outFile(end-3:end), '.xlsx')    % the extension should be .csv
%             outFile = [outFile '.xlsx'];     % so we append it to out_file_name if it does not end with '.xls'.
%         end
          
%         fid = fopen(outFile, 'w+') ;
%         [~,r] =size(handles.TabFeat.Data);
%         A=handles.TabFeat.ColumnName';
%          
%          T1={handles.TabFeat.ColumnName';handles.TabFeat.Data};
%          T = cell2table(C);
%          T.Properties.VariableNames = T1(1,:);
%          
%          
%          
%          
% %          T(1:r,:)
%          xlswrite(filename,[T1,T2])
