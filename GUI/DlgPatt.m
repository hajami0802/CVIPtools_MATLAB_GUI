...
% Last Modified by GUIDE v2.5 28-Feb-2023 19:58:33
...
% Shrink the GUI
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pos =  get(handles.Patt,'position');
% pos(4) = 33;
set(handles.Patt,'position',pos)
...
if isempty(file_out) && handles.bLeave.Value
 ...
 if handles.cVector.Value            %perform distance measure operations
    ...
    if ~isempty(file_out)      
        ...
        file_out = [pwd '\GUI\Pattern\' file_out];
    ...
    group = setfigdocked('GroupName','CVIP Toolbox V.3.7','Figure',hGeo); %Add figure to group
    ...
    if norm_data(1) == 4
        norm_data(2) = str2double(handles.txtSmin.String);
        norm_data(3) = str2double(handles.txtSmax.String);
    end
    if norm_data(1) == 5
        norm_data(2) = str2double(handles.txtSoft.String);
    end
   
    dist_data = handles.bDis.SelectedObject.UserData;
    if handles.bDSmin.Value     
        dist_data(2) = str2double(handles.popDmin.String(handles.popDmin.Value));
    end
...
%         sound(y,Fs); %display time and play sound elapTime(toc), 
        ...
        if strcmp(features.obj_id{end},'Images:')%new feature vector
            ...
            file_train = [cpath(1:end-7) 'Temp\my_train_temp.CSV'];
        ...
        if strcmp(features.obj_id{end},'Images:')%new feature vector
            ...
            features = features(1:n_feat,:);%get features
            cpath = mfilename( 'fullpath' );
            file_test = [cpath(1:end-7) 'Temp\my_test_tmp.CSV'];
...
%         if size(Dist)== [1 ]
        ...
        Class = (unique(OClass));%class names
...
% --------------------------------------------------------------------
function mEdge_Callback(hObject, eventdata, ~)
...
% --- Executes on key press with focus on bOut and none of its controls.
function bOut_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to bOut (see GCBO)
...
% --- Executes on button press in bDNran.
function bDNran_Callback(hObject, eventdata, handles)
% hObject    handle to bDNran (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of bDNran


% --- Executes on button press in bDSeuc.
function bDSeuc_Callback(hObject, eventdata, handles)
% hObject    handle to bDSeuc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of bDSeuc


% --- Executes on button press in bDNno.
function bDNno_Callback(hObject, eventdata, handles)
% hObject    handle to bDNno (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.bDStan.Visible = 'On'; %Test

% Hint: get(hObject,'Value') returns toggle state of bDNno
