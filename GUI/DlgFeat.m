...
% Last Modified by GUIDE v2.5 11-Oct-2022 14:28:50
...
% Revision History
%revision 1.6  01/20/2021  Hridoy
%line 228 : save  the name of the file to use it in the featureviwer gui
%function 
...
% Shrink the GUI
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pos =  get(handles.Feat,'position');
% pos(4) = 33;
set(handles.Feat,'position',pos)
pos =  get(handles.slider5,'position');
% pos(4) = 31;
set(handles.slider5,'position',pos)

% Create the list of the objects to move
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
handles.hListe =  [handles.text17;...
%     handles.bFeat; ...
%     handles.bHelp;...
%     handles.bFeat; ...
    handles.edit12; ...
...
%extract features
outFile = handles.txtFFile.String;
ImgFile = handles.txtOIma.String;
text=strcat(outFile ,'.csv');
setappdata(0,'name',text);
   %output filename
...
handles.txtClass.UserData = Class; 
...
if handles.cFeatIma.Value       %perform Feature Images 
    ...
    if ~handles.cAssi.Value
        ...
        Feats = feature_objects_cvip(InIma, MaskIma, objLabel, ImgFile, ...
...
function crst5_Callback(hObject, eventdata, ~)
    ...
    if isa(eventdata,'double')
        if eventdata
           ...
           hGeo = DlgFView;
            ...
            group = setfigdocked('GroupName','CVIP Toolbox V.3.61','Figure',hGeo); %Add figure to group
            DlgFView('FView_SizeChangedFcn',hObject,eventdata,guidata(hGeo)) 
        end
    else    %show features
        hGeo = findobj('Tag','FView');              %get the handle of Main form
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
text=get(object,'string');
text=char(strcat(text,'.csv'));
setappdata(0,'name',text);
...
% --- Executes on key press with focus on bApply and none of its controls.
function bApply_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to bApply (see GCBO)
...
pos = cellfun(@(x) {x-[0 get(handles.slider5,'value') 0 0]},handles.pos);
% added for stepping
set(handles.slider5, 'Min', 1);
set(handles.slider5, 'Max', 15);
% set(handles.slider1, 'Value', 1);
set(handles.slider5, 'SliderStep', [1/(10-1) , 9/(10-1) ]);
% added close
...
% --- Executes on key press with focus on TF and none of its controls.
function TF_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to TF (see GCBO)
% eventdata  structure with the following fields (see MATLAB.UI.CONTROL.UICONTROL)
%	Key: name of the key that was pressed, in lower case
%	Character: character interpretation of the key(s) that was pressed
