function [outImage, kernel] = morpherode_cvip(inImage, kType, kSize, kArgs)
% MORPHERODE_CVIP - Morphological erosion of a grayscale or color image.
% The function performs morphological erosion of a grayscale image. The 
% user can specify the kernel and its parameters, otherwise the default 
% kernel and kernel parameters are selected.
%
% Syntax :
% -------
% [outImage, struct_el] = morpherode_cvip(inImage, kType, kSize, kArgs)
%   
% 
...
% -------------------------
%   'outImage'      Output image of Morphological erosion. It is of double
%                   class.
...
% Example 1 :
% ---------
%     %Perform erosion using default parameters and get only ouput image
%       I = imread('cam.bmp');         %original image
%       O1 = morpherode_cvip(I);      %Square kernel with size 3           
%       figure;imshow(O1,[]);
% Example 2 :
% ---------
%     %Perform erosion using user specified parameters and get two 
%       %outputs (output image and kernel)
%       I = imread('cam.bmp');          %original image
%       [O2, kernel] = morpherode_cvip(I,3,5,[5 3]); %5*5 rectangle kernel
...
%center of structuring element or kernel
...
for r = -(centr_loc-1) : centr_loc -1
    for c = -(centr_loc - 1) : centr_loc - 1  
        ...
        if kernel(r + centr_loc, c + centr_loc) == 1
            if r < 0
                temp2 = [inf(-r, ncol, nband); inImage(1:end+r,:,:)];
            elseif r > 0
                temp2 = [inImage(1+r:end,:, :); inf(r,ncol, nband)];
            else
                temp2 = inImage;
            end

            if c < 0
                temp2 = [inf(nrow,-c, nband) temp2(:, 1:end+c, :)]; 
            elseif c > 0
                temp2 = [temp2(:, 1+c:end, :) inf(nrow,c,nband)];
            end
            
            %find the minimum
            outImage = min(outImage, temp2);    
