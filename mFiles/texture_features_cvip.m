function texfeats = texture_features_cvip(inImage, labelImage,out_file_name, texDist, featSelect, objLabel, quantLvl, statsType,n)
...
%   'n'             It should be 1 by default. It is for GUI function
%
%
...
% Revision History
%Revision 1.3 2/2/21  Hridoy
%  Condition is given for invalid objects which objects have value 0.Also,a warning has been created. 
%
...
if nargin ~=1 && nargin ~= 2 && nargin ~= 3 && nargin ~=4 && nargin ~=5 ...
        && nargin ~= 6 && nargin ~=7 && nargin ~=8 && nargin ~=9 
...
%labelImage=resize(objLabel(1,1),objLabel(1,2));
%k=sum(labelImage);
ro = objLabel(1,1);
co =  objLabel(1,2);

% for i=1:objLabel(1,1)
%     for j=1:objLabel(1,2)
%         sum1=sum1+labelImage(i,j);
%     end
% end
if labelImage(ro,co)==0 %
     opts = struct('WindowStyle','modal',... 
              'Interpreter','tex');
         warndlg('Give a valid objects and try to chnage the co-ordinate ','Warning',opts);
else
%%
...
if classFlag 
    if n==0
      texfeats = [  obj_id feats_title ;out_file_name num2cell(objLabel) num2cell(feature_val) ];
    else
