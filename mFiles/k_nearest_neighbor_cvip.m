...
switch option_normalize
    ...
    case 4  % Min-Max normalization
        switch option_distance
            ...
            case 4 % Minkowski 
                if nargin ~= 10
                    error(['You should have 10 input arguments when ' ...
                        'option_normalize=' num2str(option_normalize) ...
                        ' and option_distance= ' num2str(option_distance)])
                end
                s_min = varargin{1};
...
%             Nclass(i) = sum(strcmp(Tr{:,end},class{i})); 
%         end 
...
function class_names = class_fun(Test, Tr, id)
    ...
    for j=1:size(Test,1)
        ...
        for i=1:size(id,2) 
            Nclass{j,i} = Tr{id(j,i),end}{1};
        end
        for i = 1:size(id,2)
          Nclass{j,i} = find(strcmp(Nclass(j,i), class)); 
        end
        
    end
    Nclass = cell2mat(Nclass);
    for j = 1:size(Test,1)
       a(j) = mode(Nclass(j,:));
    end
    ...
    for i = 1:size(Test,1) 
        if a(i) == 1
            class_names(i) = class(Nclass(i,1),:);
        else
        class_names(i) = class(a(1,i),:); 
        end
    end 
