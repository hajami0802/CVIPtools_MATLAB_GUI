...
[m,~] = size(X);
f = waitbar(0,'K-means','Name','K means running ..',...
...
idx = zeros(m, 1);
 ...
 for i = 1:max_iters
    ...
    % Update waitbar and message
        waitbar(i/max_iters,f,sprintf('Iteration %d of %d',i,max_iters))
 end
delete(f)
end
...
function init_cent1=kMeansInitCentroids(X,b, K,init_cent,len)
     f = waitbar(0,'Initial centroid','Name','Finding_initial_centroid...',...
    'CreateCancelBtn','setappdata(gcbf,''canceling'',1)');
...
for k=1:K
 ...
 waitbar(k/K,f,sprintf('Completing Channel %d -- %d%% of 100%%',d,(k*100)/K)) 
